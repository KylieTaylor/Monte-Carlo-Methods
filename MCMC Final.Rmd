...
```{r, include=FALSE}
data <- read.csv("~/Downloads/final.dat.txt", sep="")
DATA <- as.matrix(data)
```
...
$$ \propto e^{-\frac{\beta_0^2}{50}} e^{-\frac{\lambda n}{2}\sum(y_i - log(1+exp(\beta_0 + \beta_1 x_{1i})))^2}  $$
...
In order to sample the $\lambda$ I can use a Gibbs Framework. 
...
This follows naturally for the move from model 2 to model 1, except that the $\alpha$ values are different.
...
I will take $f(\beta_2|M=1)$ to be a normal centered around 0, with a small variance, since we cannot make too many assumptions as to what the value of $\beta_2$ is from $\beta_0$ and $\beta_1$ from model 1. 
